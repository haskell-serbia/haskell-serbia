...
userForm muser =
  ...
  User <$> areq identField (bfs ("User id" :: Text)) mident <*>
  ...
  where
    ...
    identField =
      checkM
        (FH.isUniq ("User already exists" :: Text) UserIdent mident)
        textField
    nameField =
      checkM (FH.isUniq ("User name is taken" :: Text) UserName mname) textField
    roles :: [(Text, Role)]
